...
export default function Footer() {
  return (
    <footer className="text-white" style={{ backgroundColor: "rgb(0, 10, 23)" }}>
      <div className="container-main py-12 md:py-16">
        ...
        <Link href="/" className="block mb-4">
          <span
            ...
            style={{ fontSize: "clamp(40px, 10vw, 72px)", letterSpacing: "-0.02em", lineHeight: 1 }}
          >
            AQUAFUNDED
          </span>
        </Link>
        ...
        <div className="flex items-center gap-4 mb-10">
          <span className="text-sm text-white/40 shrink-0">Our socials</span>
          {socials.map((s) => (
            <a
              key={s.label}
              href="#"
        ...
        {/* Nav — 2-column grid on mobile, 4-column on lg */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          {footerColumns.map((col) => (
            <div key={col.heading}>
              <h4 className="text-sm font-semibold text-white/40 mb-4">{col.heading}</h4>
        ...
        {/* Owned by AquaFunds — full width card */}
        <div className="border border-white/15 rounded-2xl p-6 mb-8 text-center">
          <p className="text-sm text-white/70 mb-1">
            Owned by <span className="font-extrabold text-white tracking-wider">AQUAFUNDS</span>
          </p>
          ...
          <a
            href="#"
            className="block w-full text-center text-sm font-semibold text-[#0A1628] bg-white rounded-full py-3 hover:bg-white/90 transition-colors"
          >
            Visit AquaFunds
          </a>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap mb-10">
          {["VISA", "MC", "Maestro", "AMEX", "Apple Pay", "₿", "Ξ", "Digital Currencies"].map((m) => (
            <span
              key={m}
              className="text-xs font-medium text-white/30 px-2.5 py-1.5 rounded-lg bg-white/5 border border-white/10"
            >
              {m}
            </span>
          ))}
